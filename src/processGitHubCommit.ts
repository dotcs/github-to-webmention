...
export function processFileCommit(fileName: string): void {
    if (!fileName.startsWith(CONTENT_DIRECTORY || '')) {
        return;
    }
    console.log(`File created: ${fileName}`);
    const url = `${WEBSITE_URL}/${fileName.replace(`${CONTENT_DIRECTORY}/`, '').replace(/.md$/, '')}`;
...
async function pingPuSHServer(url: string): Promise<any> {
    console.log(url);
